...
def basic_evaluate_board(board: List[List[Any]], player: str, rows: int, cols: int, score_cols: List[int]) -> float:
    ...
    # 1) Immediate scoring / reachable counts
    ...
    # weights (tunable)
    W_SCORE     = 150.0   # stones already in scoring area
    W_REACH     = 100.0   # stones that can reach in one move
    W_PROGRESS  = 0.5     # distance-to-goal reward per row unit
    W_PROGRESS_OPP = 0.4  # opponent distance penalty

    score = W_SCORE*(n_self - n_opp) + W_REACH*(m_self - m_opp)

    goal_self = top_score_row() if player == "circle" else bottom_score_row(rows)
    goal_opp  = bottom_score_row(rows) if player == "circle" else top_score_row()

    for y in range(rows):
        for x in range(cols):
            p = board[y][x]
            if not p or getattr(p,"side","stone") != "stone":
                continue
            dist = abs((goal_self if p.owner == player else goal_opp) - y)
            prog = (rows - dist)
            score += W_PROGRESS * prog if p.owner == player else -W_PROGRESS_OPP * prog
...
class StudentAgent(BaseAgent):
    ...
    def choose(self, board: List[List[Any]], rows: int, cols: int, score_cols: List[int],
        ...
        for move in moves:
            src = tuple(move["from"])
            # 'to' may not exist (flip without orientation), so treat absent as None target
            dst = tuple(move.get("to", (None, None)))

            # Anti-oscillation: if piece at src last came from dst, skip
            last_from = self.last_move_info.get(src)
            if last_from and dst == last_from:
                continue
            ...
            val = basic_evaluate_board(new_board, self.player, rows, cols, score_cols)

            # Immediate win detection (evaluate_board returns +inf for wins)
            if val == float("inf"):
                best_move = move
                best_score = val
                break
        ...
        # update oscillation map keyed by destination -> source
        if best_move and "to" in best_move:
            src = tuple(best_move["from"])
            dst = tuple(best_move["to"])
            self.last_move_info[dst] = src
