            ...
            if piece.side == "stone":
                ...
                for (tx, ty) in targets.get('moves', set()):
                    if is_own_score_cell(tx, ty, player, rows, cols, score_cols):
                        m += 1
                        reachable_pieces.add((x, y))
                        break # Go to the next piece
                
                if (x, y) in reachable_pieces:
                    continue

                # Check pushes where this piece pushes another
                for _, (ptx, pty) in targets.get('pushes', []):
                    # This rule is about the player's OWN pieces reaching the scoring area.
                    # A standard push moves the pushing piece, not the pushed piece, into the scoring area.
                    # We only care if the landing spot of OUR piece is a scoring cell.
                    # The 'to' part of a push move is where our piece lands.
                    pass # This logic is covered by the `moves` check in a well-formed move generator.
...
def basic_evaluate_board(board: List[List[Any]], player: str, rows: int, cols: int, score_cols: List[int]) -> float:
    ...
    n_self = count_stones_in_scoring_area(board, player, rows, cols, score_cols)
    n_opp = count_stones_in_scoring_area(board, opponent, rows, cols, score_cols)
    m_self = count_reachable_in_one_move(board, player, rows, cols, score_cols)
    m_opp = count_reachable_in_one_move(board, opponent, rows, cols, score_cols)
    if n_self >=4:
        return float('inf')
    if n_opp >=4:
        return float('-inf')
    score += (n_self * 100 + m_self * 10) - (n_opp * 100 + m_opp * 10)

    # --- 2. Positional & Strategic Bonuses (Revised as requested) ---
    # Explicitly define both goal rows for clarity.
    if player == "circle":
        player_goal_row = top_score_row()
        opponent_goal_row = bottom_score_row(rows)
    else: # player is "square"
        player_goal_row = bottom_score_row(rows)
        opponent_goal_row = top_score_row()
    ...
    for y in range(rows):
        for x in range(cols):
            ...
            if not piece:
             ...
             # --- Heuristics for Stone Pieces ---
            if piece.side == "stone":
                if piece.owner == player:
                    # Proximity Reward: Encourage player's stones to advance towards their goal.
                    distance_to_goal = abs(y - player_goal_row)
                    score += (rows - distance_to_goal) * 0.1
                else: # Opponent's stone
                    # Proximity Penalty: Penalize opponent's stones that advance towards their goal.
                    distance_to_their_goal = abs(y - opponent_goal_row)
                    score -= (rows - distance_to_their_goal) * 0.1
            
            # --- River Strategic Value (Revised Logic) ---
            elif piece.side == "river":
                # Part A: Immediate utility based on adjacent stones
                    ...
                    if in_bounds(nx, ny, rows, cols):
                        ...
                        if neighbor and neighbor.side == "stone":
                            if neighbor.owner == player:
                                score += 0.3 # This river helps our stones
                            else:
                                score -= 0.3 # This river is a threat
                
                # Part B: Positional value based on proximity to scoring areas
                distance_to_player_goal = abs(y - player_goal_row)
                distance_to_opponent_goal = abs(y - opponent_goal_row)

                # Add a reward for being close to our goal (the opponent's side)
                score += (rows - distance_to_player_goal) * 0.2
                
                # Add a penalty for being close to the opponent's goal (our side)
                score -= (rows - distance_to_opponent_goal) * 0.2
...
class StudentAgent(BaseAgent):
    ...
    def choose(self, board: List[List[Any]], rows: int, cols: int, score_cols: List[int],
        ...
        """
        Analyzes all possible moves for the current turn and selects the one that leads
        to the highest-scoring board state, penalizing immediate move reversals.
        """
        ...
        for move in moves:
            ...
            # ADDED: Check for and penalize immediate move reversals.
            fr = tuple(move.get("from", []))
            to = tuple(move.get("to", []))

            # If a piece at `fr` just came from `to` on the last turn, penalize moving back.
            if fr in self.last_move_info and self.last_move_info[fr] == to:
                score -= 5  # Apply a penalty to discourage oscillation

            if score > best_score:
                best_score = score
                best_move = move

        # ADDED: Update last_move_info with the chosen move.
        self.last_move_info.clear() # Clear previous turn's move data.
        if best_move:
            fr = tuple(best_move.get("from", []))
            to = tuple(best_move.get("to", []))
            # Only track moves that involve a piece changing its position.
            if fr and to:
                # The piece that is NOW at `to` WAS at `fr`.
